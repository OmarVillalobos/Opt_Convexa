...
fit <- brm(Approved_Conversion ~ age + gender + interest + Spent , data = sales, family = bernoulli(link = "logit"), 
           prior = get_prior(Approved_Conversion ~ age + gender + interest + Spent , data = sales, family = poisson()), 
           silent=TRUE, 
           refresh = -1)
...
fit <- brm(Approved_Conversion ~ age + gender + interest + Spent , data = sales, family = bernoulli(link = "logit"), 
           prior = c(empty_prior(), 
                     empty_prior(),
                     set_prior("normal(-2,1)", class="b", coef="interest"),
                     set_prior("normal(-1,1)", class="b", coef="Spent")), 
           silent=TRUE, 
           refresh = -1)
prior <- get_prior(Approved_Conversion ~ age + gender + interest + Spent , data = sales, family = poisson())
make_stancode(Approved_Conversion ~ age + gender + interest + Spent , data = sales, family = poisson(), prior = prior)
```

```{r}
library(ggplot2)

ggplot(sales , aes(x = interest, y = Clicks, size = Clicks, color = interest)) + 
 # geom_boxplot() +
...
add_spend <- function(spend){
  ...
  our.phone <- data.frame(age = '30-34', gender='M', interest=32, Spent = spend) 
...
optimize(function(x) add_spend(x), c(-6000,6000), maximum = TRUE)
